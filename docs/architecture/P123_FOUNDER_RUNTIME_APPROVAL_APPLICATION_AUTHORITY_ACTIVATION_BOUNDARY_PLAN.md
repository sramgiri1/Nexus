...
Phase: P123 Founder Runtime Approval Application Authority Activation Boundary
Subphase: P123.7 Final Validation
Goal: close P123 with final validation, complete P123.1-P123.7 status, and
create the planned P124 handoff without enabling live authority.
Why this is needed: final validation prevents stale status, verifies all P123
evidence and reports, and gives the next phase a safe planned entry.
User/operator impact: operators can see P123 as complete and P124 as the next
planned OS phase while activation remains blocked.
Command Center impact: preserve P123.5 activation UX only. P123.7 adds no new
Command Center source, routes, controls, labels, or tests.
Safety impact: final validation only. It cannot activate authority, grant
authority, apply approvals, accept approvals, persist approvals, record
approve/reject decisions, write DB/runtime records, unlock execution, dispatch
agents, run workers/tools, mutate projects, call providers/models, use hosted
DBs, deploy, release, export, package, use network calls, or spend.
...
Expected exports, schemas, and data shapes:
- No runtime exports.
- P123.7 checker report uses the shared markdown report shape.
- P123/P123.7 status records must have real commits after the stamp commit.
- P124 is a planned handoff entry only.

Reuse check: reuse P123.6 validation, P123.5 scoped UX evidence, P123.4
preview evidence, `shared/reportWriter.js`, `shared/checkResultFormatter.js`,
`shared/reportMetadata.js`, `os-roadmap/updatePhaseStatus.js`, existing
dashboard cards/routes, route tests, and OS phase status records. Do not
duplicate UI cards, report writers, mode guards, redaction helpers, checker
formatters, result envelopes, route matrices, phase status updaters, or
evidence/activity appenders.

Command Center UX requirements: preserve the activation boundary card on
Business Build and Agent Flow only. Chat with NEXUS, Lite, Command Center home,
OS Roadmap, and Live Readiness remain free of the activation card. Primary UX
must not show raw JSON, raw logs, raw policy dumps, raw report paths, raw
private IDs, internal phase labels outside OS Roadmap, DemoApp, mutation
controls, or fake working actions.

Dark/light/system theme requirements: preserve System, Dark, and Light theme
behavior. No CSS or theme token changes in P123.7.

Playwright tests: no new Playwright test file edits. Run the existing focused
activation route coverage for scoped page visibility and theme modes.

Tests to add/update/remove: add P123.7 final checker and package script. Update
the OS phase status checker to recognize P124 as a future current phase. No
tests are removed.

Checker updates: validate P123.1-P123.7 scripts, reports, docs/status, parent
P123 completion, P124 planned handoff, P123.5/P123.6 checker handoff
acceptance, no stale pending commits, forbidden path boundaries, and unsafe
positive claim prevention.
...
OS phase status update: P123 is complete; P123.7 is complete; current phase
P123.7; previous P123.6; next P124; P124 is planned.
...
Git add/commit/push commands:
- `git add <allowed P123.7 files>`
- `git commit -m "feat(nexus): implement p1237 approval authority activation final validation"`
- stamp P123/P123.7 status with the implementation commit
- `git add <allowed P123.7 status/report files>`
- `git commit -m "chore(nexus): stamp p1237 approval authority activation final validation"`
- `git push origin codex/nexus-e2e-phase-validation`
...
Known risks: P124 may be mistaken for implemented work. The P124 entry is
planned only and has no runtime behavior, no dashboard route, no DB/runtime
write, and no provider spend.

Rollback plan: remove the P123.7 checker, package script, report, P124
allowlist/planned entry, docs/status updates, restore P123 to in progress,
restore P123.7 to planned, and return current phase to P123.6.
...
Validation / Docs are complete. P123.7 Final Validation is complete, P123 is
complete, and P124 is next as a planned handoff only. Activation, authority
grant, approval
