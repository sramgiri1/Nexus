...
| policy-center | WARDEN + AUDITOR | Policy registry, versioning, diff, simulation, exception, and break-glass previews | `policy-center/*` | Shared utilities, policy metadata, Command Center view data | Runtime policy override, provider/tool dispatch, DB writes, worker execution, project mutation | `check:policy-center-final-validation` | Policy Center |
| secrets | WARDEN + AUDITOR | Secret reference model, access policy previews, redaction scanner, provider/project credential boundaries | `secrets/*` | Shared utilities, policy metadata, Command Center view data | Raw secret reads, environment value resolution, provider/tool dispatch, DB writes, deploy, mobile signing, project mutation | `check:secrets-boundary-final-validation` | Secrets Boundary |
...
| worker-runtime | SENTINEL + WARDEN | Preview-only worker queue, leases, heartbeats, retry/timeout, and dead-letter primitives | `worker-runtime/*` | Shared utilities, policy metadata, local reports | Worker loops, arbitrary commands, provider/tool execution, DB writes, project mutation | `check:worker-runtime` | Worker Runtime |
