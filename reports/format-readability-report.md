...
- Timestamp: 2026-05-06T09:48:41.885Z
...
- docs/architecture/AGENTIC_OS_ARCHITECTURE.md:360 (296) — The state machine is the authoritative record of task, gate, and project state. Agents propose state transitions. The state machine validates and commits them. 
...
- docs/architecture/AGENTIC_OS_ARCHITECTURE.md:79 (288) — Without domain ownership, multiple agents write to the same artifacts. CORE rewrites a schema that ATLAS owns. SENTINEL approves a gate that AUDITOR should have
- docs/architecture/AGENTIC_OS_ARCHITECTURE.md:381 (288) — An agent that writes `"status": "completed"` to a task is proposing a transition, not committing one. The state machine checks: does the result contain verifiab
- docs/architecture/AGENTIC_OS_ARCHITECTURE.md:115 (286) — An agent that builds an API route has good judgment about whether that route is correct. It does not have authority to decide the route is verified. Judgment an
- docs/architecture/AGENTIC_OS_ARCHITECTURE.md:178 (283) — A vague handoff — "CORE is done, SENTINEL should do QA now" — is not a contract. It contains no scope constraints, no acceptance criteria, no skill requirements
...
- docs/architecture/AGENTIC_OS_ARCHITECTURE.md:564 (280) — CareLoop parallel execution should not begin until agents have been confirmed as contract-aware, state-machine-aware, model-aware, batch-aware, skill-aware, and
...
- docs/architecture/AGENTIC_OS_ARCHITECTURE.md:429 (272) — The governor runs inside `executeTool` before the tool handler executes. There is no code path that runs a tool without passing through `authorizeAction()`. Age
