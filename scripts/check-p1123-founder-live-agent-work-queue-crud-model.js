...
const REPORT_PATH = "reports/p1123-founder-live-agent-work-queue-crud-model-report.md";
const TEST_DB = "local-state/runtime/check-p1123.sqlite";
...
const moduleSource = readText("live-ready/founderLiveAgentWorkQueueAdmission.js");
const p1123 = contract.subphases?.find((entry) => entry.phaseId === "P112.3") || {};
const p1124 = contract.subphases?.find((entry) => entry.phaseId === "P112.4") || {};
const blockedWorkflow = buildFounderLiveAgentWorkQueueAdmissionContract();
const readyWorkflow = buildFounderLiveAgentWorkQueueAdmissionContract({
...
const blockedValidation = validateFounderLiveAgentWorkQueueAdmissionContract(blockedWorkflow);
const readyValidation = validateFounderLiveAgentWorkQueueAdmissionContract(readyWorkflow);
const defaultBlocked = executeApprovedAgentWorkQueueDbCrudRequest({ sqliteEntity: "founder_agent_work_queue_items", requestKey: "default-blocked" });
const unapprovedBlocked = executeApprovedAgentWorkQueueDbCrudRequest(
  { sqliteEntity: "founder_agent_work_queue_items", requestKey: "unapproved" },
  { execute: true, mode: "sqlite-live", enableWrites: true },
);
const deleteBlocked = executeApprovedAgentWorkQueueDbCrudRequest(
  { sqliteEntity: "founder_agent_work_queue_items", requestKey: "delete" },
...
const outsideAllowlistBlocked = executeApprovedAgentWorkQueueDbCrudRequest(
  { sqliteEntity: "founder_agent_work_orders", requestKey: "outside" },
...
if (cliAvailable) {
  ...
  for (const entity of P112_AGENT_WORK_QUEUE_DB_ENTITIES) {
    writeResults.push(executeApprovedAgentWorkQueueDbCrudRequest(
      { sqliteEntity: entity, requestKey: `write-${entity}` },
      { ...sqliteInput, operation: "upsert", record: buildSafeAgentWorkQueueDbRecord(entity, { queueKey: "p1123-check", timestamp: "2026-05-28T00:00:00.000Z" }) },
    ));
  }
  readResult = executeApprovedAgentWorkQueueDbCrudRequest(
    { sqliteEntity: "founder_agent_work_queue_items", requestKey: "read-entry" },
    { ...sqliteInput, operation: "read", id: "p1123-queue-item-p1123-check" },
  );
  updateResult = executeApprovedAgentWorkQueueDbCrudRequest(
    { sqliteEntity: "founder_agent_work_queue_items", requestKey: "update-entry" },
    { ...sqliteInput, operation: "update", id: "p1123-queue-item-p1123-check", patch: { queueState: "updated_locally", updatedAt: "2026-05-28T00:00:00.000Z" } },
  );
  listResult = executeApprovedAgentWorkQueueDbCrudRequest(
    { sqliteEntity: "founder_agent_work_queue_items", requestKey: "list-entry" },
...
addCheck("package script registered", Boolean(packageJson.scripts?.["check:p1123-founder-live-agent-work-queue-crud-model"]));
addCheck("phase export is P112.3", P112_FOUNDER_LIVE_AGENT_WORK_QUEUE_ADMISSION_PHASE === "P112.3");
addCheck("allowed entity list is scoped", P112_AGENT_WORK_QUEUE_DB_ENTITIES.length === 3 && P112_AGENT_WORK_QUEUE_DB_ENTITIES.every((entity) => entity.startsWith("founder_agent_work_queue")));
...
addCheck("SQLite CLI available", cliAvailable, "sqlite3 command is required for P112.3 CRUD validation");
addCheck("isolated DB initializes", !cliAvailable || (init.initialized === true && existsSync(join(ROOT, TEST_DB))));
addCheck("approved local CRUD writes queue records", !cliAvailable || writeResults.length === 3 && writeResults.every((result) => result.ok === true && result.admitted === true && result.written === true && result.hostedDbWritesAllowed === false && result.projectMutationAllowed === false));
addCheck("approved local CRUD reads queue record", !cliAvailable || readResult?.record?.queueItemId === "p1123-queue-item-p1123-check");
addCheck("approved local CRUD updates queue record", !cliAvailable || updateResult?.record?.queueState === "updated_locally");
addCheck("approved local CRUD lists queue records", !cliAvailable || listResult?.records?.some((entry) => entry.queueItemId === "p1123-queue-item-p1123-check"));
...
addCheck("module reuses work order helper", moduleSource.includes("buildSafeAgentWorkOrderDbRecord"));
addCheck("contract marks P112.3 complete", p1123.status === "complete" && ["planned", "complete"].includes(p1124.status));
addCheck("docs record P112.3", /P112\.3 Governed Local Queue CRUD Model[\s\S]*Status:\s+complete/.test(plan));
addCheck("README records P112.3", /P112\.3 governed local queue CRUD model/.test(readme) && /P112\.4\s+is\s+next/.test(readme));
addCheck("platform roadmap records P112.3", /P112\.3 is complete/.test(platformRoadmap) && /P112\.4 is next/.test(platformRoadmap));
addCheck(
  "phase status advanced",
  ["P112.3", "P112.4"].includes(status.currentPhase)
    && ["P112.2", "P112.3"].includes(status.previousPhase)
    && ["P112.4", "P112.5"].includes(status.nextPhase)
    && ["P112.3", "P112.4"].includes(roadmap.currentPhase)
    && ["P112.2", "P112.3"].includes(roadmap.previousPhase)
    && ["P112.4", "P112.5"].includes(roadmap.nextPhase)
    && statusById.get("P112.3")?.status === "complete"
    && ["planned", "complete"].includes(statusById.get("P112.4")?.status)
    && ["planned", "complete"].includes(statusById.get("P112.5")?.status)
    && roadmapById.get("P112.3")?.status === "complete",
