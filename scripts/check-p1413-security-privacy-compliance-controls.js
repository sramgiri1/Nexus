...
addCheck("contract advances to P141.3 safely", contract.phaseId === "P141" && contract.status === "in_progress" && contract.currentSubphase === "P141.3" && contract.previousSubphase === "P141.2" && contract.nextSubphase === "P141.4");
addCheck("contract records expected base commit", p1413.expectedBaseCommit === EXPECTED_BASE_COMMIT);
addCheck("P141.2 complete, P141.3 complete, P141.4 planned", p1412.status === "complete" && p1413.status === "complete" && p1414.status === "planned");
...
addCheck("OS checker recognizes P141.4 handoff", ["P141.1", "P141.2", "P141.3", "P141.4"].every((phaseId) => osStatusChecker.includes(`"${phaseId}"`)));
...
addCheck("enterprise roadmap records P141.3", /P141\.3 is now complete/i.test(enterpriseRoadmap) && /P141\.4 is the next executable subphase/i.test(enterpriseRoadmap));
addCheck("phase status advances to P141.3", p1413CurrentState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P141.4 remains planned-only", statusById.get("P141.4")?.status === "planned" && roadmapById.get("P141.4")?.status === "planned" && !(statusById.get("P141.4")?.checksRun || []).length && !(roadmapById.get("P141.4")?.checksRun || []).length);
