...
addCheck("phase status closes P140.7", p1407FinalState || p1411StartedState || p1412CurrentState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P141 handoff remains valid", (statusById.get("P141")?.status === "planned" && roadmapById.get("P141")?.status === "planned" && !(statusById.get("P141")?.checksRun || []).length && !(roadmapById.get("P141")?.checksRun || []).length) || p1411StartedState || p1412CurrentState);
