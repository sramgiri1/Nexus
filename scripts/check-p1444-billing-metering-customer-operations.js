...
addCheck("contract advances to P144.4 safely", contract.phaseId === "P144" && contract.status === "in_progress" && contract.currentSubphase === "P144.4" && contract.previousSubphase === "P144.3" && contract.nextSubphase === "P144.5" && p1444.status === "complete" && p1445.status === "planned");
...
addCheck("enterprise checker accepts P144.4 active state", enterpriseChecker.includes("p1444CurrentState") && enterpriseChecker.includes(REQUIRED_SCRIPT));
...
addCheck("docs record P144.4 and P144.5 handoff", /## P144\.4 Customer Operations Command Center UX[\s\S]*Status:\s+complete/.test(plan) && /P144\.4\s+Customer\s+Operations\s+Command\s+Center\s+UX\s+is\s+complete/i.test(readme) && /P144\.4\s+Customer\s+Operations\s+Command\s+Center\s+UX\s+is\s+complete/i.test(platformRoadmap) && /P144\.4 is now complete as a Cost Center Customer Ops UX/i.test(enterpriseRoadmap) && /P144\.5 is planned-only next/i.test(enterpriseRoadmap));
addCheck("phase status advances to P144.4", p1444CurrentState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("next P144.5/P145 handoff remains planned-only", [statusById.get("P144.5"), roadmapById.get("P144.5"), statusById.get("P145"), roadmapById.get("P145")].every((entry) => entry?.status === "planned" && entry.commit === "" && Array.isArray(entry.checksRun) && entry.checksRun.length === 0));
addCheck("P144.4 Playwright coverage exists", routeTests.includes("P144.4 customer operations UX keeps Cost Center useful and non-runnable") && routeTests.includes("Customer Operations Readiness") && routeTests.includes("P144.4") && routeTests.includes("Customer Operations Command Center UX") && routeTests.includes("P144.5"));
