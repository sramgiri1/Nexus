...
addCheck("contract advances to P143.4 safely", contract.phaseId === "P143" && contract.status === "in_progress" && contract.currentSubphase === "P143.4" && contract.previousSubphase === "P143.3" && contract.nextSubphase === "P143.5" && p1433.status === "complete" && p1434.status === "complete" && p1435.status === "planned");
...
addCheck("phase status advances to P143.4", p1434CurrentState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("next P143.5/P144 handoff remains planned-only", [statusById.get("P143.5"), roadmapById.get("P143.5"), statusById.get("P144"), roadmapById.get("P144")].every((entry) => entry?.status === "planned" && entry.commit === "" && Array.isArray(entry.checksRun) && entry.checksRun.length === 0));
...
addCheck("changed files stay in P143.4 allowed scope", changed.every((file) => allowedFiles.has(file)), changed.join(", "));
