...
addCheck("contract keeps P145.4 complete through handoff", contract.phaseId === "P145" && p1454.status === "complete" && ((contract.status === "in_progress" && (p1454CurrentState || (contract.currentSubphase === "P145.5" && contract.previousSubphase === "P145.4" && contract.nextSubphase === "P145.6" && p1455.status === "complete") || (contract.currentSubphase === "P145.6" && contract.previousSubphase === "P145.5" && contract.nextSubphase === "P145.7" && p1455.status === "complete" && p1456.status === "complete"))) || (p1457FinalState && contract.status === "complete" && contract.currentSubphase === "P145.7" && contract.previousSubphase === "P145.6" && contract.nextSubphase === "" && p1457.status === "complete")));
...
addCheck("P145.5 handoff is safe", (p1454CurrentState && p1455.status === "planned" && p1455.expectedBaseCommit === "after-P145.4" && p1455.commit === "" && Array.isArray(p1455.checksRun) && p1455.checksRun.length === 0) || ((p1455CurrentState || p1456CurrentState || p1457FinalState) && p1455.status === "complete" && p1455.expectedBaseCommit === "4412d7fd" && Array.isArray(p1455.checksRun) && p1455.checksRun.includes(`npm run ${NEXT_SCRIPT}`)));
...
addCheck("phase status keeps P145.4 complete through handoff", p1454CurrentState || p1455CurrentState || p1456CurrentState || p1457FinalState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P145.5 remains safe", (p1454CurrentState && [statusById.get("P145.5"), roadmapById.get("P145.5")].every((entry) => entry?.status === "planned" && entry.commit === "" && Array.isArray(entry.checksRun) && entry.checksRun.length === 0)) || (p1455CurrentState && [statusById.get("P145.5"), roadmapById.get("P145.5")].every((entry) => entry?.status === "complete" && Boolean(entry.commit) && Array.isArray(entry.checksRun) && entry.checksRun.includes(`npm run ${NEXT_SCRIPT}`)) && [statusById.get("P145.6"), roadmapById.get("P145.6")].every((entry) => entry?.status === "planned" && entry.commit === "" && Array.isArray(entry.checksRun) && entry.checksRun.length === 0)) || (p1456CurrentState && [statusById.get("P145.5"), roadmapById.get("P145.5")].every((entry) => entry?.status === "complete" && Boolean(entry.commit) && Array.isArray(entry.checksRun) && entry.checksRun.includes(`npm run ${NEXT_SCRIPT}`)) && [statusById.get("P145.6"), roadmapById.get("P145.6")].every((entry) => entry?.status === "complete" && Boolean(entry.commit) && Array.isArray(entry.checksRun) && entry.checksRun.includes("npm run check:p1456-enterprise-ga-readiness-docs-roadmap"))) || p1457FinalState);
