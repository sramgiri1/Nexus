...
addCheck("contract advances P140.4 safely", contract.phaseId === "P140" && contract.status === "in_progress" && contract.currentSubphase === "P140.4" && contract.previousSubphase === "P140.3" && contract.nextSubphase === "P140.5");
addCheck("contract records expected base commit", p1404.expectedBaseCommit === EXPECTED_BASE_COMMIT);
addCheck("P140.4 complete and P140.5 handoff known", p1404.status === "complete" && p1405.status === "planned" && p1404.nextPhase === "P140.5");
...
addCheck("OS checker recognizes P140.5 handoff", osStatusChecker.includes('"P140.5"'));
...
addCheck("enterprise roadmap records P140.4", /P140\.4 is now complete/i.test(enterpriseRoadmap) && /P140\.5 is the next executable subphase/i.test(enterpriseRoadmap));
addCheck("phase status keeps P140.4 complete", p1404CurrentState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P140.5 handoff remains planned-only", statusById.get("P140.5")?.status === "planned" && roadmapById.get("P140.5")?.status === "planned" && !(statusById.get("P140.5")?.checksRun || []).length && !(roadmapById.get("P140.5")?.checksRun || []).length);
