...
} from "../shared/founderApprovalApplicationAuthorityActivationPreview.js";

const ROOT = process.cwd();
const REPORT_PATH = "reports/p1234-founder-runtime-approval-application-authority-activation-boundary-report.md";
...
function hasUnsafePositiveClaim(text, pattern) {
  ...
  return lines.some((line, index) => {
    ...
    return pattern.test(line)
      && !/\b(no|not|never|without|blocked|unavailable|disabled|forbidden|do not|does not|remain|remains|dry-run|preview-only|planned-only|read-only|future)\b/i.test(context);
  });
}
...
const p1233Checker = readText("scripts/check-p1233-founder-runtime-approval-application-authority-activation-boundary.js");
const previewSource = readText("shared/founderApprovalApplicationAuthorityActivationPreview.js");
const preview = buildFounderApprovalApplicationAuthorityActivationPreview({
  ...
  nextAction: "Render P123.5 scoped activation boundary UX without activate, grant, apply, approve, reject, save, or execution controls.",
});
const validation = validateFounderApprovalApplicationAuthorityActivationPreview(preview);
const data = preview.data || {};
const summary = data.activationSummary || {};
const invalidValidation = validateFounderApprovalApplicationAuthorityActivationPreview({
  ...preview,
  data: {
    ...data,
    dryRunOnly: false,
...
const enforceCurrentDiffScope = status.currentPhase === "P123.4";
const allowedFiles = new Set(p1234.allowedFiles || []);
...
const blockedFlagNames = [
  ...
  "approvalApplicationAuthorityActivationWriteAllowed",
  "approvalApplicationAuthorityActivationPersistenceAllowed",
  "approvalApplicationAuthorityActivationRuntimeAdmissionAllowed",
  "approvalApplicationAuthorityActivationOperatorOverrideAllowed",
  "approvalApplicationAuthorityActivationSpendAllowed",
  "approvalApplicationAuthorityActivationPreviewAllowed",
  "approvalApplicationAuthorityActivationPreviewWritten",
  "authorityActivated",
  "authorityGranted",
  "activationPersisted",
...
const p1234HandoffAccepted = (status.currentPhase === "P123.4"
...
addCheck("package script registered", Boolean(packageJson.scripts?.["check:p1234-founder-runtime-approval-application-authority-activation-boundary"]));
addCheck("preview exports exist", previewSource.includes("buildFounderApprovalApplicationAuthorityActivationPreview") && previewSource.includes("validateFounderApprovalApplicationAuthorityActivationPreview"));
addCheck("phase and version exports", FOUNDER_APPROVAL_APPLICATION_AUTHORITY_ACTIVATION_PREVIEW_PHASE === "P123.4" && FOUNDER_APPROVAL_APPLICATION_AUTHORITY_ACTIVATION_PREVIEW_VERSION === "1.0");
addCheck("preview states are allowlisted", Object.values(FOUNDER_APPROVAL_APPLICATION_AUTHORITY_ACTIVATION_PREVIEW_STATES).includes("activation preview ready; activation blocked") && !Object.values(FOUNDER_APPROVAL_APPLICATION_AUTHORITY_ACTIVATION_PREVIEW_STATES).includes("activation_live"));
...
addCheck("preview envelope shape", preview.ok === true && preview.status === "PASS" && data.schemaVersion === "1.0" && data.previewMode === "local-only-authority-activation-dry-run" && data.dryRunOnly === true);
addCheck("preview stays Command Center hidden", data.commandCenterVisible === false);
addCheck("preview reuses P123.3 intent model and P123.2 metadata", data.sourceIntentPhase === "P123.3" && data.sourceMetadataPhase === "P123.2" && previewSource.includes("buildFounderApprovalApplicationAuthorityActivationIntentModel") && previewSource.includes("buildFounderApprovalApplicationAuthorityActivationEligibilityMetadata"));
...
addCheck("row authority flags false", data.previewRows?.every((row) => flagsFalse(row) && row.wouldActivateAuthority === false && row.wouldGrantAuthority === false && row.wouldApplyDecision === false && row.wouldCaptureApproval === false && row.wouldPersistApproval === false && row.wouldRecordDecision === false && row.wouldAcceptDecision === false && row.wouldRejectDecision === false && row.wouldWriteDb === false && row.wouldWriteRuntime === false && row.wouldUnlockExecution === false && row.wouldDispatchAgent === false && row.wouldExecuteWorker === false && row.wouldExecuteTool === false && row.wouldMutateProject === false && row.wouldUseNetwork === false && row.wouldSpend === false));
...
addCheck("contract marks P123.4 complete", p1234.status === "complete" && ["planned", "complete"].includes(p1235.status) && ["planned", "complete"].includes(p1236.status));
addCheck("contract records expected exports", [
  "FOUNDER_APPROVAL_APPLICATION_AUTHORITY_ACTIVATION_PREVIEW_PHASE",
  "buildFounderApprovalApplicationAuthorityActivationPreview",
  "validateFounderApprovalApplicationAuthorityActivationPreview",
].every((name) => p1234.expectedExports?.includes(name)));
addCheck("P123.3 checker accepts P123.4 handoff", p1233Checker.includes("P123.4") && p1233Checker.includes("P123.5") && p1233Checker.includes("p1234StartedState"));
addCheck("docs record P123.4", /P123\.4 Activation Safe Dry Run[\s\S]*Status:\s+complete/.test(plan));
addCheck("README records P123.4", /P123\.4 approval application authority activation safe dry run/i.test(readme) && /P123\.5\s+is\s+next/.test(readme));
addCheck("platform roadmap records P123.4", /P123\.4 is complete/.test(platformRoadmap) && /P123\.5\s+is\s+next/.test(platformRoadmap));
addCheck(
  "phase status advanced",
  p1234HandoffAccepted
    && statusById.get("P123")?.status === "in_progress"
    && statusById.get("P123.3")?.status === "complete"
    && statusById.get("P123.4")?.status === "complete"
    && ["planned", "complete"].includes(statusById.get("P123.5")?.status)
    && roadmapById.get("P123.4")?.status === "complete",
  `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`,
);
addCheck(
  "changed files stay in P123.4 allowed scope",
...
addCheck(
  ...
  enforceCurrentDiffScope ? changed.join(", ") : `P123.4 forbidden path check relaxed for ${status.currentPhase}`,
);
...
addCheck("preview avoids raw schema/table names", !/(founderApprovalApplicationAuthorityActivation|approval_authority_activation)/i.test(serializedPreview));
addCheck("preview avoids fake runnable actions", !/activate now|grant authority now|approve now|reject now|save decision now|run now|execute now|deploy now|apply now|call provider now|create project now|dispatch agent now|write sqlite now|write approval now|unlock execution now/i.test(serializedPreview));
...
writeMarkdownReport(
  ...
  [
    ...
    { title: "Validation Commands", body: p1234.validationCommands.map((command) => `- ${command}`).join("\n") },
    {
      title: "Known Limitations",
      body: "- P123.4 is a local dry-run preview only. It does not activate authority, grant authority, apply approvals, accept approvals, persist approvals, record approve/reject decisions, write DB/runtime records, unlock execution, run runtime work, call providers/models, dispatch agents, execute workers/tools, mutate projects, deploy, release, export, package, use network calls, or spend.",
    },
    { title: "Result", body: failed.length === 0 ? `PASS (${checks.length}/${checks.length})` : `FAIL (${failed.length} failed)` },
  ],
  { title: "P123.4 Approval Application Authority Activation Safe Dry Run Report", phase: "P123.4" },
);

printCheckReport("P123.4 Approval Application Authority Activation Safe Dry Run Check", checks, failed.length === 0 ? "PASS" : "FAIL");
