...
addCheck("platform roadmap records P91.3", platformRoadmap.includes("P91.3 is complete") && platformRoadmap.includes("P91.4 is next"));
addCheck(
  "phase status advanced",
  statusById.get("P91")?.status === "in_progress"
    && statusById.get("P91.3")?.status === "complete"
    && status.currentPhase === "P91.3"
    && status.previousPhase === "P91.2"
    && status.nextPhase === "P91.4",
  `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`,
);
addCheck("roadmap tracks P91.3", roadmapById.get("P91.3")?.track === "NEXUS_OS" && roadmapById.get("P91.3")?.status === "complete");
addCheck("P91.4 planned handoff exists", statusById.get("P91.4")?.status === "planned" && roadmapById.get("P91.4")?.status === "planned");
