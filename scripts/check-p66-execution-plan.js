...
const CONTRACT_PATH = "contracts/os-roadmap/p66-execution-contracts.json";
const PLAN_PATH = "docs/architecture/P66_SELF_HEALING_FAILURE_LOOP_PLAN.md";
const STATUS_PATH = "os-roadmap/phase-status.json";
const REPORT_PATH = "reports/p66-execution-plan-report.md";

export const P66_SELF_HEALING_SUBPHASES = ["P66.1", "P66.2", "P66.3", "P66.4", "P66.5", "P66.6", "P66.7"];
...
export function checkP66ExecutionPlan() {
  const checks = {
    contractFile: true,
    taskContracts: true,
    requiredFields: true,
  ...
  if (tasks.length !== P66_SELF_HEALING_SUBPHASES.length) {
    fail("taskContracts", `Expected ${P66_SELF_HEALING_SUBPHASES.length} task contracts, found ${tasks.length}`);
  }
  ...
  for (const task of tasks) {
    ...
    if (!hasAllText(inputs.safetyRules || [], forbiddenTerms)) {
      fail("safetyRules", `${task.id} safety rules must block mutation, execution, automatic retry, DB, deploy, and spend`);
    }
    ...
    if (!hasCommand(inputs.validationCommands || [], "npm run check:p66-execution-plan")) {
      fail("validationCommands", `${task.id} must include npm run check:p66-execution-plan`);
    }
    ...
    if (!hasAllText(inputs.finalSafetyChecks || [], forbiddenTerms)) {
      fail("finalSafety", `${task.id} final safety must cover mutation, execution, automatic retry, DB, deploy, and spend`);
    }
  ...
  for (const phaseId of P66_SELF_HEALING_SUBPHASES) {
  ...
  const p66 = phaseStatus.get("P66");
  const p661 = phaseStatus.get("P66.1");
  const p662 = phaseStatus.get("P66.2");
  const p663 = phaseStatus.get("P66.3");
  const p664 = phaseStatus.get("P66.4");
  const p665 = phaseStatus.get("P66.5");
  const p666 = phaseStatus.get("P66.6");
  const p667 = phaseStatus.get("P66.7");
  const expectedNext =
    p667?.status === "complete"
      ? "P67"
      : p666?.status === "complete"
        ? "P66.7"
        : p665?.status === "complete"
          ? "P66.6"
          : p664?.status === "complete"
            ? "P66.5"
            : p663?.status === "complete"
              ? "P66.4"
              : p662?.status === "complete"
                ? "P66.3"
                : p661?.status === "complete"
                  ? "P66.2"
                  : "P66.1";
  if (!["in_progress", "complete"].includes(p66?.status)) fail("roadmapStatus", "P66 must be in_progress or complete");
  if (p66?.nextPhase !== expectedNext) fail("roadmapStatus", `P66 nextPhase must be ${expectedNext}`);
  if (p667?.status === "complete") {
    if (status.currentPhase !== "P67") fail("roadmapStatus", "currentPhase must be P67 after P66 closure");
    if (status.previousPhase !== "P66") fail("roadmapStatus", "previousPhase must be P66 after P66 closure");
    if (status.nextPhase !== "P68") fail("roadmapStatus", "nextPhase must be P68 after P66 closure");
  } else {
    if (status.currentPhase !== "P66") fail("roadmapStatus", "currentPhase must be P66");
    if (status.previousPhase !== "P65") fail("roadmapStatus", "previousPhase must be P65");
  ...
  writeMarkdownReport(
    ...
    [
      { title: "Contract", body: [`- Path: ${CONTRACT_PATH}`, `- Subphases: ${P66_SELF_HEALING_SUBPHASES.join(", ")}`].join("\n") },
    ...
    { title: "P66 Execution Plan Report", phase: "P66.1" },
  );

  return { rows, failures, result: failures.length === 0 ? "PASS" : "FAIL" };
}

const result = checkP66ExecutionPlan();
printCheckReport("P66 Execution Plan Check", result.rows, result.result);
