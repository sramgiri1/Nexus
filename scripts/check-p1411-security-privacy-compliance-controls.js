...
addCheck("contract starts P141 safely", contract.phaseId === "P141" && contract.status === "in_progress" && contract.currentSubphase === "P141.1" && contract.previousSubphase === "P140.7" && contract.nextSubphase === "P141.2");
...
addCheck("P141.1 complete and P141.2 planned", p1411.status === "complete" && p1412.status === "planned");
...
addCheck("OS checker recognizes P141 subphases", ["P141", "P141.1", "P141.2"].every((phaseId) => osStatusChecker.includes(`"${phaseId}"`)));
...
addCheck("enterprise roadmap records P141.1", /P141\.1 is now complete/i.test(enterpriseRoadmap) && /P141\.2 is the next executable subphase/i.test(enterpriseRoadmap));
addCheck("phase status starts P141.1", p1411StartedState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P141.2 remains planned-only", statusById.get("P141.2")?.status === "planned" && roadmapById.get("P141.2")?.status === "planned" && !(statusById.get("P141.2")?.checksRun || []).length && !(roadmapById.get("P141.2")?.checksRun || []).length);
