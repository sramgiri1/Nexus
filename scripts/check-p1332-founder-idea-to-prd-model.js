...
addCheck("contract marks P133.2 complete", contract.status === "in_progress" && contract.currentSubphase === "P133.2" && contract.previousSubphase === "P133.1" && contract.nextSubphase === "P133.3" && p1332.status === "complete");
...
addCheck("P133.3 handoff remains planned-only", p1333.status === "planned" && statusById.get("P133.3")?.status === "planned" && roadmapById.get("P133.3")?.status === "planned");
...
addCheck("enterprise roadmap records P133.2", (/P133\.2 is now complete/i.test(enterpriseRoadmap) || /P133\.1 and P133\.2 are now complete/i.test(enterpriseRoadmap)) && /P133\.3 is the next executable subphase/i.test(enterpriseRoadmap));
addCheck("phase status advanced", p1332CompleteState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
