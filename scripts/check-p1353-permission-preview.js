...
addCheck("contract records P135.3 completion", contract.currentSubphase === "P135.3" && contract.previousSubphase === "P135.2" && contract.nextSubphase === "P135.4" && p1353.status === "complete" && p1354.status === "planned");
...
addCheck("enterprise roadmap records P135.3", /P135\.3 is now complete/i.test(enterpriseRoadmap) && /P135\.4 is the next executable subphase/i.test(enterpriseRoadmap));
addCheck("phase status starts P135.3", p1353CurrentState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P135.4 remains planned-only", statusById.get("P135.4")?.status === "planned" && roadmapById.get("P135.4")?.status === "planned" && !(statusById.get("P135.4")?.checksRun || []).length);
addCheck("changed files stay in P135.3 allowed scope", changed.every((file) => allowedFiles.has(file)), changed.join(", "));
