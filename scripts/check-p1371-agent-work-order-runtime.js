...
addCheck("contract starts P137 safely", contract.phaseId === "P137" && contract.status === "in_progress" && contract.currentSubphase === "P137.1" && contract.previousSubphase === "P136.7" && contract.nextSubphase === "P137.2");
...
addCheck("P137.1 complete and P137.2 planned", p1371.status === "complete" && p1372.status === "planned");
...
addCheck("OS checker recognizes P137 subphases", ["P137", "P137.1", "P137.2"].every((phaseId) => osStatusChecker.includes(`"${phaseId}"`)));
...
addCheck("enterprise roadmap records P137.1", /P137\.1 is now complete/i.test(enterpriseRoadmap) && /P137\.2 is the next executable subphase/i.test(enterpriseRoadmap));
addCheck("phase status starts P137.1", p1371StartedState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P137.2 remains planned-only", statusById.get("P137.2")?.status === "planned" && roadmapById.get("P137.2")?.status === "planned" && !(statusById.get("P137.2")?.checksRun || []).length);
addCheck(
