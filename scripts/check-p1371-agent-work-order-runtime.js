...
addCheck("contract starts P137 safely", contract.phaseId === "P137" && contract.status === "in_progress" && ((contract.currentSubphase === "P137.1" && contract.previousSubphase === "P136.7" && contract.nextSubphase === "P137.2") || (contract.currentSubphase === "P137.2" && contract.previousSubphase === "P137.1" && contract.nextSubphase === "P137.3")));
...
addCheck("P137.1 complete and P137.2 planned or complete", p1371.status === "complete" && (p1372.status === "planned" || (p1372.status === "complete" && p1373.status === "planned")));
...
addCheck("OS checker recognizes P137 subphases", ["P137", "P137.1", "P137.2", "P137.3"].every((phaseId) => osStatusChecker.includes(`"${phaseId}"`)));
...
addCheck("phase status starts P137.1 or safely hands off to P137.2", p1371StartedState || p1372CurrentState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P137.2 remains planned or safely complete", (statusById.get("P137.2")?.status === "planned" && roadmapById.get("P137.2")?.status === "planned" && !(statusById.get("P137.2")?.checksRun || []).length) || p1372CurrentState);
addCheck(
...
writeMarkdownReport(
  ...
  [
    ...
    { title: "Validation Commands", body: VALIDATION_COMMANDS.map((command) => `- ${command}`).join("\n") },
    {
      ...
      body: "- P137.1 is contract/policy/safety-boundary work only. It does not create work-order runtime models, dispatch dry runs, Agent Flow UX, DB/runtime writes, provider/model calls, tool execution, MCP startup, agent dispatch, project mutation, deploy, release, export, package, network calls, or spend. P137.2 remains planned-only.",
    },
