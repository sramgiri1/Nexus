...
const CONTRACT_PATH = "contracts/os-roadmap/p74-execution-contracts.json";
const PLAN_PATH = "docs/architecture/P74_OBSERVABILITY_TELEMETRY_SLOS_PLAN.md";
const STATUS_PATH = "os-roadmap/phase-status.json";
const REPORT_PATH = "reports/p74-execution-plan-report.md";

export const P74_OBSERVABILITY_SUBPHASES = ["P74.1", "P74.2", "P74.3", "P74.4", "P74.5", "P74.6", "P74.7"];
...
export function checkP74ExecutionPlan() {
  const checks = {
    contractFile: true,
    taskContracts: true,
    safetyRules: true,
  ...
  if (tasks.length !== P74_OBSERVABILITY_SUBPHASES.length) fail("taskContracts", `Expected ${P74_OBSERVABILITY_SUBPHASES.length} task contracts, found ${tasks.length}`);
  ...
  for (const task of tasks) {
    ...
    if (!hasAllText(inputs.safetyRules || [], ["telemetry", "DB writes", "project mutation", "provider", "tool execution", "network", "provider spend"])) {
      fail("safetyRules", `${task.id} safety rules must block telemetry export, DB writes, project mutation, provider/tool execution, network calls, and spend`);
    }
    ...
    if (!(inputs.validationCommands || []).includes("npm run check:p74-execution-plan")) fail("validationCommands", `${task.id} must include npm run check:p74-execution-plan`);
  }
  for (const phaseId of P74_OBSERVABILITY_SUBPHASES) {
  ...
  const p74 = phaseStatus.get("P74");
  const expectedNext = P74_OBSERVABILITY_SUBPHASES.find((phaseId) => phaseStatus.get(phaseId)?.status !== "complete") || "P75";
  const finalHandoff = p74?.status === "complete" && status.currentPhase === "P75" && status.previousPhase === "P74" && status.nextPhase === "P75";
  if (!["in_progress", "complete"].includes(p74?.status)) fail("roadmapStatus", "P74 must be in_progress or complete");
  if (p74?.nextPhase !== expectedNext) fail("roadmapStatus", `P74 nextPhase must be ${expectedNext}`);
  if (!finalHandoff && status.currentPhase !== "P74") fail("roadmapStatus", "currentPhase must be P74 before final handoff");
  if (!finalHandoff && status.previousPhase !== "P73") fail("roadmapStatus", "previousPhase must be P73 before final handoff");
  ...
  if (!/telemetry[\s\S]+remain disabled/i.test(plan) || !/SLO[\s\S]+disabled/i.test(plan)) {
    fail("docs", `${PLAN_PATH} must state telemetry and SLO runtime behavior remain disabled`);
  }
  ...
  writeMarkdownReport(
    REPORT_PATH,
    [
      { title: "Contract", body: [`- Path: ${CONTRACT_PATH}`, `- Subphases: ${P74_OBSERVABILITY_SUBPHASES.join(", ")}`].join("\n") },
    ...
    { title: "P74 Execution Plan Report", phase: "P74.1" },
  );
  return { rows, failures, result: failures.length === 0 ? "PASS" : "FAIL" };
}

const result = checkP74ExecutionPlan();
printCheckReport("P74 Execution Plan Check", result.rows, result.result);
