...
addCheck("contract marks P134.5 complete", (contract.status === "in_progress" || contract.status === "complete") && ((contract.currentSubphase === "P134.5" && contract.previousSubphase === "P134.4" && contract.nextSubphase === "P134.6") || (contract.currentSubphase === "P134.6" && contract.previousSubphase === "P134.5" && contract.nextSubphase === "P134.7") || (contract.currentSubphase === "P134.7" && contract.previousSubphase === "P134.6" && contract.nextSubphase === "P135")) && p1345.status === "complete");
...
addCheck("enterprise roadmap records P134.5", /P134\.5 is now complete/i.test(enterpriseRoadmap) && (/P134\.6 is the next executable subphase/i.test(enterpriseRoadmap) || /P134\.7 is the next executable subphase/i.test(enterpriseRoadmap) || /P135 is the next executable phase/i.test(enterpriseRoadmap)));
addCheck("phase status advanced", p1345CurrentState || p1346CurrentState || p1347FinalState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("changed files stay in P134.5 allowed scope", p1346CurrentState || p1347FinalState || changed.every((file) => allowedFiles.has(file) || file === REPORT_PATH), (p1346CurrentState || p1347FinalState) ? `scope check relaxed for ${status.currentPhase}` : changed.join(", "));
addCheck("forbidden paths unchanged", p1346CurrentState || p1347FinalState || changed.every((file) => allowedDashboardFiles.has(file) || !forbiddenPrefixes.some((prefix) => file.startsWith(prefix))), (p1346CurrentState || p1347FinalState) ? `P134.5 forbidden path check relaxed for ${status.currentPhase}` : changed.join(", "));
