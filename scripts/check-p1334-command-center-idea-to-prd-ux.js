...
addCheck("contract marks P133.4 complete", contract.status === "in_progress" && contract.currentSubphase === "P133.4" && contract.previousSubphase === "P133.3" && contract.nextSubphase === "P133.5" && p1334.status === "complete");
...
addCheck("P133.5 handoff remains planned-only", p1335.status === "planned" && statusById.get("P133.5")?.status === "planned" && roadmapById.get("P133.5")?.status === "planned");
...
addCheck("enterprise and P132.7 checkers accept P133.4", enterpriseChecker.includes("p1334CompleteState") && enterpriseChecker.includes("check:p1334-command-center-idea-to-prd-ux") && p1327Checker.includes("p1334CompleteState") && p1327Checker.includes('status.currentPhase === "P133.4"'));
...
addCheck("enterprise roadmap records P133.4", (/P133\.4 is now complete/i.test(enterpriseRoadmap) || /P133\.1, P133\.2, P133\.3, and P133\.4 are now complete/i.test(enterpriseRoadmap)) && /P133\.5 is the next executable subphase/i.test(enterpriseRoadmap));
addCheck("phase status advanced", p1334CompleteState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
