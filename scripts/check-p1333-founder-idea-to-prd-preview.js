...
addCheck("contract marks P133.3 complete", contract.status === "in_progress" && contract.currentSubphase === "P133.3" && contract.previousSubphase === "P133.2" && contract.nextSubphase === "P133.4" && p1333.status === "complete");
...
addCheck("P133.4 handoff remains planned-only", p1334.status === "planned" && statusById.get("P133.4")?.status === "planned" && roadmapById.get("P133.4")?.status === "planned");
...
addCheck("enterprise roadmap records P133.3", (/P133\.3 is now complete/i.test(enterpriseRoadmap) || /P133\.1, P133\.2, and P133\.3 are now complete/i.test(enterpriseRoadmap)) && /P133\.4 is the next executable subphase/i.test(enterpriseRoadmap));
addCheck("phase status advanced", p1333CompleteState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
writeMarkdownReport(
  ...
  [
    ...
    { title: "Validation Commands", body: validationCommands.map((command) => `- ${command}`).join("\n") },
    {
      ...
      body: "- P133.3 is an in-memory read-only preview only. Command Center rendering starts in P133.4, and live execution remains blocked.",
    },
