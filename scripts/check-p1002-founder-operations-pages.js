...
addCheck("P100.2 contract complete with P100.3 handoff", p1002?.status === "complete" && subphaseById.get("P100.3")?.status === "planned");
...
addCheck(
  ...
  statusById.get("P100")?.status === "in_progress"
    ...
    && status.currentPhase === "P100.2"
    && status.previousPhase === "P100.1"
    && status.nextPhase === "P100.3",
  `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`,
);
addCheck("roadmap tracks P100.2", roadmapById.get("P100.2")?.track === "NEXUS_OS" && roadmapById.get("P100.2")?.status === "complete");
addCheck("P100.3 handoff exists", statusById.get("P100.3")?.status === "planned" && roadmapById.get("P100.3")?.status === "planned");
