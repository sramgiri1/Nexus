...
const REPORT_PATH = "reports/p1396-evidence-audit-observability-cost-ledger-report.md";
...
const REQUIRED_SCRIPT = "check:p1396-evidence-audit-observability-cost-ledger";
const EXPECTED_BASE_COMMIT = "c15cb7b4";
...
const checkerSource = readText("scripts/check-p1396-evidence-audit-observability-cost-ledger.js");
...
const enforceCurrentDiffScope = status.currentPhase === "P139.6";
const allowedFiles = new Set(p1396.allowedFiles || []);
...
addCheck("package script registered", packageJson.scripts?.[REQUIRED_SCRIPT] === "node scripts/check-p1396-evidence-audit-observability-cost-ledger.js");
addCheck("checker reuses shared report helpers", checkerSource.includes("../shared/reportWriter.js") && checkerSource.includes("../shared/checkResultFormatter.js"));
addCheck("P139.1-P139.6 package scripts registered", ["p1391", "p1392", "p1393", "p1394", "p1395", "p1396"].every((suffix) => Boolean(packageJson.scripts?.[`check:${suffix}-evidence-audit-observability-cost-ledger`])));
addCheck("P139.1-P139.5 reports pass", [
...
addCheck("prior P139 checkers accept P139.6", [p1391Checker, p1392Checker, p1393Checker, p1394Checker, p1395Checker].every((source) => source.includes("p1396CurrentState") && source.includes('status.currentPhase === "P139.6"')));
addCheck("enterprise checker accepts P139.6", enterpriseChecker.includes("p1396CurrentState") && enterpriseChecker.includes(REQUIRED_SCRIPT));
addCheck("OS checker recognizes P139.7 handoff", ["P139.1", "P139.2", "P139.3", "P139.4", "P139.5", "P139.6", "P139.7"].every((phaseId) => osStatusChecker.includes(`"${phaseId}"`)));
addCheck("contract advances P139.6", contract.phaseId === "P139" && ((contract.status === "in_progress" && contract.currentSubphase === "P139.6" && contract.previousSubphase === "P139.5" && contract.nextSubphase === "P139.7" && p1397.status === "planned") || (contract.status === "complete" && contract.currentSubphase === "P139.7" && contract.previousSubphase === "P139.6" && contract.nextSubphase === "P140" && p1397.status === "complete")) && p1396.status === "complete");
addCheck("contract records expected base commit", p1396.expectedBaseCommit === EXPECTED_BASE_COMMIT);
addCheck("contract records complete validation commands", VALIDATION_COMMANDS.every((command) => p1396.validationCommands?.includes(command)));
addCheck("contract scope stays docs/status-only", p1396.dataShape?.includes("Docs/status/report updates only") && p1396.expectedExports?.length === 0 && p1396.forbiddenFiles?.includes("dashboard/src/**") && p1396.forbiddenFiles?.includes("projects/**"));
addCheck("P139 plan records P139.6", /## P139\.6 Docs \/ Roadmap \/ Status[\s\S]*Status:\s+complete/.test(plan) && (/P139\.7 remains planned-only next/i.test(plan) || /P139\.7 is complete; P139 remains planned-only next/i.test(plan)));
addCheck("README records P139.6", /P139\.6 docs\/roadmap\/status closure/i.test(readme) && (/P139\.7 is planned-only next/i.test(readme) || /P139\.7 and left P139 planned-only next/i.test(readme)));
addCheck("platform roadmap records P139.6", /P139\.6 docs\/roadmap\/status closure is complete/i.test(platformRoadmap));
addCheck("enterprise roadmap records P139.6", /P139\.6 is now complete/i.test(enterpriseRoadmap) && (/P139\.7 is the next executable subphase/i.test(enterpriseRoadmap) || /P139\.7 is now complete/i.test(enterpriseRoadmap)));
addCheck("phase status starts P139.6 or hands off to P139.7", p1396CurrentState || p1397FinalState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
addCheck("completed P139.6 entries have required fields", [statusById.get("P139"), statusById.get("P139.6"), roadmapById.get("P139.6")].every((entry) => Boolean(entry?.phaseId) && Boolean(entry.branch) && Boolean(entry.commit) && Boolean(entry.summary) && Array.isArray(entry.checksRun) && Array.isArray(entry.knownLimitations)));
addCheck("P139.7 remains planned or is safely complete", (statusById.get("P139.7")?.status === "planned" && roadmapById.get("P139.7")?.status === "planned" && !(statusById.get("P139.7")?.checksRun || []).length) || p1397FinalState);
addCheck("changed files stay in P139.6 allowed scope", !enforceCurrentDiffScope || changed.every((file) => allowedFiles.has(file)), enforceCurrentDiffScope ? changed.join(", ") : `scope check relaxed for ${status.currentPhase}`);
addCheck("forbidden paths unchanged", !enforceCurrentDiffScope || changed.every((file) => !forbiddenPrefixes.some((prefix) => file.startsWith(prefix))), enforceCurrentDiffScope ? changed.join(", ") : `P139.6 forbidden path check relaxed for ${status.currentPhase}`);
  ...
  [
    ...
    },
    {
      ...
      body: [
        ...
        `- Prior P139 reports passing: ${["P139.1", "P139.2", "P139.3", "P139.4", "P139.5"].length}`,
      ].join("\n"),
    },
    ...
    { title: "Validation Commands", body: VALIDATION_COMMANDS.map((command) => `- ${command}`).join("\n") },
    {
      ...
      body: "- P139.6 is docs/status/report closure only. It does not enable live ledger persistence, audit writes, evidence writes, observability writes, cost writes, DB/runtime writes, provider/model calls, tool execution, MCP startup, agent dispatch, project mutation, deploy, release, export, package, network calls, or spend. P139.7 remains planned-only.",
    },
    { title: "Result", body: failed.length === 0 ? `PASS (${checks.length}/${checks.length})` : `FAIL (${failed.length} failed)` },
  ],
  { title: "P139.6 Evidence Audit Observability Cost Ledger Docs Status Report", phase: "P139.6" },
);

printCheckReport("P139.6 Evidence Audit Observability Cost Ledger Docs Status Check", checks, failed.length === 0 ? "PASS" : "FAIL");
