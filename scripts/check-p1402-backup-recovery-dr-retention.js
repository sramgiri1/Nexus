...
addCheck("contract advances P140.2 safely", contract.phaseId === "P140" && ((contract.status === "in_progress" && ((contract.currentSubphase === "P140.2" && contract.previousSubphase === "P140.1" && contract.nextSubphase === "P140.3") || (contract.currentSubphase === "P140.3" && contract.previousSubphase === "P140.2" && contract.nextSubphase === "P140.4") || (contract.currentSubphase === "P140.4" && contract.previousSubphase === "P140.3" && contract.nextSubphase === "P140.5") || (contract.currentSubphase === "P140.5" && contract.previousSubphase === "P140.4" && contract.nextSubphase === "P140.6") || (contract.currentSubphase === "P140.6" && contract.previousSubphase === "P140.5" && contract.nextSubphase === "P140.7"))) || (contract.status === "complete" && p1407FinalState && contract.currentSubphase === "P140.7" && contract.previousSubphase === "P140.6" && contract.nextSubphase === "P141")));
addCheck("contract records expected base commit", p1402.expectedBaseCommit === EXPECTED_BASE_COMMIT && typeof contract.expectedBaseCommit === "string" && contract.expectedBaseCommit.length > 0);
addCheck("P140.2 complete and P140.3 handoff known", p1402.status === "complete" && ["planned", "complete"].includes(p1403.status) && p1402.nextPhase === "P140.3");
...
addCheck("OS checker recognizes P140.3 handoff", osStatusChecker.includes('"P140.3"') && osStatusChecker.includes('"P140.4"') && osStatusChecker.includes('"P140.5"') && osStatusChecker.includes('"P140.6"') && osStatusChecker.includes('"P140.7"') && osStatusChecker.includes('"P141"'));
...
addCheck("enterprise roadmap records P140.2", /P140\.2 is now complete/i.test(enterpriseRoadmap) && (/P140\.3 is the next executable subphase/i.test(enterpriseRoadmap) || /P140\.3 is now complete/i.test(enterpriseRoadmap)));
addCheck("phase status keeps P140.2 complete", p1402CurrentState || p1403CurrentState || p1404CurrentState || p1405CurrentState || p1406CurrentState || p1407FinalState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P140.3 handoff remains valid", (p1402CurrentState && statusById.get("P140.3")?.status === "planned" && roadmapById.get("P140.3")?.status === "planned" && !(statusById.get("P140.3")?.checksRun || []).length && !(roadmapById.get("P140.3")?.checksRun || []).length) || (p1403CurrentState && p1403.status === "complete" && p1404.status === "planned") || (p1404CurrentState && p1403.status === "complete" && p1404.status === "complete" && p1405.status === "planned") || (p1405CurrentState && p1403.status === "complete" && p1404.status === "complete" && p1405.status === "complete" && p1406.status === "planned") || (p1406CurrentState && p1403.status === "complete" && p1404.status === "complete" && p1405.status === "complete" && p1406.status === "complete" && p1407.status === "planned") || (p1407FinalState && ["P140.3", "P140.4", "P140.5", "P140.6", "P140.7"].every((phaseId) => statusById.get(phaseId)?.status === "complete" && roadmapById.get(phaseId)?.status === "complete") && statusById.get("P141")?.status === "planned"));
