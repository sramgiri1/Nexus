...
addCheck("contract advances P138.3", contract.phaseId === "P138" && contract.status === "in_progress" && contract.currentSubphase === "P138.3" && contract.previousSubphase === "P138.2" && contract.nextSubphase === "P138.4" && p1383.status === "complete" && p1384.status === "planned");
...
addCheck("OS checker recognizes P138.4 handoff", ["P138.1", "P138.2", "P138.3", "P138.4"].every((phaseId) => osStatusChecker.includes(`"${phaseId}"`)));
...
addCheck("enterprise roadmap records P138.3", /P138\.3 is now complete/i.test(enterpriseRoadmap) && /P138\.4 is the next executable subphase/i.test(enterpriseRoadmap));
addCheck("phase status starts P138.3", p1383CurrentState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P138.4 remains planned", statusById.get("P138.4")?.status === "planned" && roadmapById.get("P138.4")?.status === "planned" && !(statusById.get("P138.4")?.checksRun || []).length);
addCheck(
