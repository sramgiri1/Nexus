...
addCheck("contract keeps P143.1 complete", contract.phaseId === "P143" && contract.status === "in_progress" && contract.currentSubphase === "P143.1" && contract.previousSubphase === "P142.7" && contract.nextSubphase === "P143.2" && p1431.status === "complete" && p1432.status === "planned");
...
addCheck("P143.2 handoff is planned-only", p1432.status === "planned" && p1432.expectedBaseCommit === "after-P143.1" && p1432.commit === "" && Array.isArray(p1432.checksRun) && p1432.checksRun.length === 0);
...
addCheck("docs record P143.1 and P143.2 handoff", /## P143\.1 Contract \/ Policy \/ Safety Boundary[\s\S]*Status:\s+complete/.test(plan) && /P143\.1\s+Release\s+Deploy\s+Export\s+Package\s+Pipeline\s+Contract\s+is\s+complete/i.test(readme) && /P143\.1\s+release\s+pipeline\s+contract\s+is\s+complete/i.test(platformRoadmap) && /P143\.1 is now complete as contract\/policy\/safety-boundary only/i.test(enterpriseRoadmap) && /P143\.2 is planned-only next/i.test(enterpriseRoadmap));
addCheck("phase status starts P143.1", p1431StartedState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
addCheck("P143 parent records active status", [p143, p143Roadmap].every((entry) => entry.status === "in_progress" && entry.commit && Array.isArray(entry.checksRun) && entry.checksRun.includes("npm run check:p1431-release-deploy-export-package-pipeline") && entry.commandCenterVisible === true));
...
addCheck("next P143.2/P144 handoff remains planned-only", [statusById.get("P143.2"), roadmapById.get("P143.2"), statusById.get("P144"), roadmapById.get("P144")].every((entry) => entry?.status === "planned" && entry.commit === "" && Array.isArray(entry.checksRun) && entry.checksRun.length === 0));
