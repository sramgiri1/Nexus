import {
  P77_3_REQUIRED_FIELDS,
  P77_3_SAMPLE_PREVIEWS,
  buildAuditTrailExportPreviewEnvelope,
  createAuditTrailExportPreview,
  validateAuditTrailExportPreview,
} from "../compliance/p77-3-placeholder.js";
...
const REPORT_PATH = "reports/p773-report.md";
const checks = [];
...
const generatedPreview = createAuditTrailExportPreview({
  sourceIndex,
  evidenceRefs: ["reports/p773-report.md"],
  activityRefs: ["os-roadmap/phase-status.json#P77.3"],
});
const previews = [...P77_3_SAMPLE_PREVIEWS, generatedPreview];
const validations = previews.map((preview) => validateAuditTrailExportPreview(preview));
const envelope = buildAuditTrailExportPreviewEnvelope({
  sourceIndex,
  evidenceRefs: ["reports/p773-report.md"],
  activityRefs: ["os-roadmap/phase-status.json#P77.3"],
});
...
addCheck("required fields listed", P77_3_REQUIRED_FIELDS.length >= 39, `${P77_3_REQUIRED_FIELDS.length} fields`);
...
addCheck("certification and attestation disabled", previews.every((preview) => preview.certificationAllowed === false && preview.legalAttestationAllowed === false));
...
addCheck("private IDs tokens URLs and raw dumps hidden", !/(?:project|private|token|tenant|workspace)_[A-Za-z0-9_-]*\d[A-Za-z0-9_-]*/.test(serialized) && !/Bearer\s+|jwt|id_token|access_token|https:\/\/|postgres(?:ql)?:\/\/|raw log dump/i.test(serialized));
...
addCheck("cost impact visible", previews.every((preview) => preview.costImpact.includes("No audit export jobs")));
addCheck("no fake runnable audit action", previews.every((preview) => !/export audit|download package|create package|raw log dump|certify now|execute now/i.test(preview.disabledReason)));
addCheck("envelope pass", envelope.status === "PASS" && envelope.phase === "P77.3" && envelope.data.preview.auditExportAllowed === false);

const failed = checks.filter((check) => check.status === "FAIL");
writeMarkdownReport(
  REPORT_PATH,
  [
    { title: "Scope", body: "- Validates P77.3 preview-only audit trail export preview records.\n- Does not enable audit export, raw log export, package creation, certification, legal attestation, DB writes, project mutation, tenant/access/auth/session/user/workspace mutation, provider/tool/worker execution, network calls, deploy, release, export, or provider spend." },
    { title: "Checks", body: buildCheckTable(checks) },
    { title: "Preview Shape", body: P77_3_REQUIRED_FIELDS.map((field) => `- ${field}`).join("\n") },
    { title: "Result", body: failed.length === 0 ? "PASS" : `FAIL (${failed.length} failed)` },
  ],
  { title: "P77.3 Audit Trail Export Preview Report", phase: "P77.3" },
);

printCheckReport("P77.3 Audit Trail Export Preview Check", checks, failed.length === 0 ? "PASS" : "FAIL");
