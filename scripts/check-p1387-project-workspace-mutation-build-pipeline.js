...
const REPORT_PATH = "reports/p1387-project-workspace-mutation-build-pipeline-report.md";
...
const REQUIRED_SCRIPT = "check:p1387-project-workspace-mutation-build-pipeline";
const EXPECTED_BASE_COMMIT = "8e9628f6";
...
const checkerSource = readText("scripts/check-p1387-project-workspace-mutation-build-pipeline.js");
...
const p1386Checker = readText("scripts/check-p1386-project-workspace-mutation-build-pipeline.js");
const routeTests = readText("dashboard/tests/routes.spec.js");
const changed = changedFiles();
const enforceCurrentDiffScope = status.currentPhase === "P138.7";
const allowedFiles = new Set(p1387.allowedFiles || []);
...
addCheck("package script registered", packageJson.scripts?.[REQUIRED_SCRIPT] === "node scripts/check-p1387-project-workspace-mutation-build-pipeline.js");
addCheck("checker reuses shared report helpers", checkerSource.includes("../shared/reportWriter.js") && checkerSource.includes("../shared/checkResultFormatter.js"));
addCheck("P138.1-P138.7 package scripts registered", ["p1381", "p1382", "p1383", "p1384", "p1385", "p1386", "p1387"].every((suffix) => Boolean(packageJson.scripts?.[`check:${suffix}-project-workspace-mutation-build-pipeline`])));
addCheck("P138.1-P138.6 reports pass", [
...
addCheck("prior P138 checkers accept P138.7", [p1381Checker, p1382Checker, p1383Checker, p1384Checker, p1385Checker, p1386Checker].every((source) => source.includes("p1387FinalState") && source.includes('status.currentPhase === "P138.7"')));
addCheck("enterprise checker accepts P138.7", enterpriseChecker.includes("p1387FinalState") && enterpriseChecker.includes(REQUIRED_SCRIPT));
addCheck("OS checker recognizes P138.7 current", osStatusChecker.includes('"P138.7"'));
addCheck("contract closes P138.7", contract.phaseId === "P138" && contract.status === "complete" && contract.currentSubphase === "P138.7" && contract.previousSubphase === "P138.6" && contract.nextSubphase === "P139" && p1387.status === "complete");
addCheck("contract records expected base commit", p1387.expectedBaseCommit === EXPECTED_BASE_COMMIT);
addCheck("contract records final validation commands", VALIDATION_COMMANDS.every((command) => p1387.validationCommands?.includes(command)));
addCheck("contract scope stays final-validation-only", p1387.dataShape?.includes("Final validation only") && p1387.expectedExports?.length === 0 && p1387.forbiddenFiles?.includes("dashboard/src/**") && p1387.forbiddenFiles?.includes("projects/**"));
addCheck("P138 plan records P138.7", /## P138\.7 Final Validation[\s\S]*Status:\s+complete/.test(plan) && /P139 remains planned-only next/i.test(plan));
addCheck("README records P138.7", /P138\.7 final validation/i.test(readme) && /P139 is planned-only next/i.test(readme));
addCheck("platform roadmap records P138.7", /P138\.7 final validation is complete/i.test(platformRoadmap));
addCheck("enterprise roadmap records P138.7", /P138\.7 is now complete/i.test(enterpriseRoadmap) && /P139 is the next executable phase/i.test(enterpriseRoadmap));
addCheck("phase status closes P138.7", p1387FinalState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
addCheck("completed P138.7 entries have required fields", [statusById.get("P138"), statusById.get("P138.7"), roadmapById.get("P138.7")].every((entry) => Boolean(entry?.phaseId) && Boolean(entry.branch) && Boolean(entry.commit) && Boolean(entry.summary) && Array.isArray(entry.checksRun) && Array.isArray(entry.knownLimitations)));
addCheck("P139 remains planned-only", statusById.get("P139")?.status === "planned" && roadmapById.get("P139")?.status === "planned" && !(statusById.get("P139")?.checksRun || []).length);
addCheck("changed files stay in P138.7 allowed scope", !enforceCurrentDiffScope || changed.every((file) => allowedFiles.has(file)), enforceCurrentDiffScope ? changed.join(", ") : `scope check relaxed for ${status.currentPhase}`);
addCheck("forbidden paths unchanged", !enforceCurrentDiffScope || changed.every((file) => !forbiddenPrefixes.some((prefix) => file.startsWith(prefix))), enforceCurrentDiffScope ? changed.join(", ") : `P138.7 forbidden path check relaxed for ${status.currentPhase}`);
  ...
  [
    ...
    },
    {
      ...
      body: [
        ...
        `- Prior P138 reports passing: ${["P138.1", "P138.2", "P138.3", "P138.4", "P138.5", "P138.6"].length}`,
      ].join("\n"),
    },
    ...
    { title: "Validation Commands", body: VALIDATION_COMMANDS.map((command) => `- ${command}`).join("\n") },
    {
      ...
      body: "- P138.7 is final validation only. It does not enable project mutation, patch application, build/test execution, rollback execution, DB/runtime writes, provider/model calls, tool execution, MCP startup, agent dispatch, deploy, release, export, package, network calls, or spend. P139 remains planned-only.",
    },
    { title: "Result", body: failed.length === 0 ? `PASS (${checks.length}/${checks.length})` : `FAIL (${failed.length} failed)` },
  ],
  { title: "P138.7 Project Workspace Mutation Build Pipeline Final Validation Report", phase: "P138.7" },
);

printCheckReport("P138.7 Project Workspace Mutation Build Pipeline Final Validation Check", checks, failed.length === 0 ? "PASS" : "FAIL");
