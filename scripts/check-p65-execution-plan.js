...
const CONTRACT_PATH = "contracts/os-roadmap/p65-execution-contracts.json";
const PLAN_PATH = "docs/architecture/P65_BATCH_INTELLIGENCE_JOBS_PLAN.md";
const STATUS_PATH = "os-roadmap/phase-status.json";
const REPORT_PATH = "reports/p65-execution-plan-report.md";
export const P65_BATCH_INTELLIGENCE_SUBPHASES = ["P65.1", "P65.2", "P65.3", "P65.4", "P65.5", "P65.6", "P65.7"];
...
const forbiddenTerms = ["provider upload", "batch submission", "execution", "project mutation", "DB", "deploy"];
...
export function checkP65ExecutionPlan() {
  const checks = {
    contractFile: true,
    taskContracts: true,
    requiredFields: true,
  ...
  if (tasks.length !== P65_BATCH_INTELLIGENCE_SUBPHASES.length) {
    fail("taskContracts", `Expected ${P65_BATCH_INTELLIGENCE_SUBPHASES.length} task contracts, found ${tasks.length}`);
  }
  ...
  for (const task of tasks) {
    ...
    if (!hasAllText(inputs.safetyRules || [], forbiddenTerms)) {
      fail("safetyRules", `${task.id} safety rules must block provider upload, batch submission, execution, project mutation, DB, and deploy`);
    }
    ...
    if (!hasCommand(inputs.validationCommands || [], "npm run check:p65-execution-plan")) {
      fail("validationCommands", `${task.id} must include npm run check:p65-execution-plan`);
    }
    ...
    if (!hasAllText(inputs.finalSafetyChecks || [], forbiddenTerms)) {
      fail("finalSafety", `${task.id} final safety must cover provider upload, batch submission, execution, project mutation, DB, and deploy`);
    }
  ...
  for (const phaseId of P65_BATCH_INTELLIGENCE_SUBPHASES) {
  ...
  const p65 = phaseStatus.get("P65");
  const p651 = phaseStatus.get("P65.1");
  const p652 = phaseStatus.get("P65.2");
  const p653 = phaseStatus.get("P65.3");
  const expectedNext = p653?.status === "complete" ? "P65.4" : p652?.status === "complete" ? "P65.3" : p651?.status === "complete" ? "P65.2" : "P65.1";
  ...
  writeMarkdownReport(
    ...
    [
      { title: "Contract", body: [`- Path: ${CONTRACT_PATH}`, `- Subphases: ${P65_BATCH_INTELLIGENCE_SUBPHASES.join(", ")}`].join("\n") },
    ...
    { title: "P65 Execution Plan Report", phase: "P65.1" },
  );

  return { rows, failures, result: failures.length === 0 ? "PASS" : "FAIL" };
}

const result = checkP65ExecutionPlan();
printCheckReport("P65 Execution Plan Check", result.rows, result.result);
