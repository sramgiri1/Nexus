...
} from "../shared/founderApprovalDecisionSchemaMetadata.js";

const ROOT = process.cwd();
const REPORT_PATH = "reports/p1192-founder-runtime-approval-decision-recording-boundary-report.md";
...
const p1191Checker = readText("scripts/check-p1191-founder-runtime-approval-decision-recording-boundary-contract.js");
const helperSource = readText("shared/founderApprovalDecisionSchemaMetadata.js");
const metadata = buildFounderApprovalDecisionSchemaMetadata();
const changed = changedFiles();
const enforceCurrentDiffScope = status.currentPhase === "P119.2";
const allowedFiles = new Set(p1192.allowedFiles || []);
...
const allAuthorityBlocked = Object.values(FOUNDER_APPROVAL_DECISION_AUTHORITY_FLAGS).every((value) => value === false);
...
const entityNamesMatch = JSON.stringify(FOUNDER_APPROVAL_DECISION_ENTITY_NAMES) === JSON.stringify(metadata.entities.map((entity) => entity.entityName));
...
addCheck("decision metadata blocks approve/reject", metadata.writePolicy?.approveDecisionAllowed === false && metadata.writePolicy?.rejectDecisionAllowed === false && metadata.writePolicy?.decisionPersistenceAllowed === false);
addCheck("request metadata has founder-useful fields", Boolean(metadata.entities.find((entity) => entity.entityName === "founderApprovalDecisionRequests")?.fields?.founderQuestion) && Boolean(metadata.entities.find((entity) => entity.entityName === "founderApprovalDecisionRequests")?.fields?.disabledReason));
addCheck("event metadata has audit fields", Boolean(metadata.entities.find((entity) => entity.entityName === "founderApprovalDecisionEvents")?.fields?.eventSummary) && Boolean(metadata.entities.find((entity) => entity.entityName === "founderApprovalDecisionEvents")?.fields?.rollbackLabel));
addCheck("evidence metadata has redaction fields", metadata.entities.find((entity) => entity.entityName === "founderApprovalDecisionEvidenceRefs")?.fields?.redactionRequired === "boolean");
...
addCheck("P119.1 checker accepts P119.2 handoff", p1191Checker.includes("P119.2") && p1191Checker.includes("P119.3") && p1191Checker.includes("scope check relaxed"));
addCheck("docs record P119.2", /P119\.2 Approval Decision Schema Metadata[\s\S]*Status:\s+complete/.test(plan));
addCheck("README records P119.2", /P119\.2 approval decision schema metadata/i.test(readme) && /P119\.3\s+is\s+next/.test(readme));
addCheck("platform roadmap records P119.2", /P119\.2 is complete/.test(platformRoadmap) && /P119\.3\s+is\s+next/.test(platformRoadmap));
addCheck(
  ...
  ((status.currentPhase === "P119.2"
    ...
    && ["planned", "complete"].includes(statusById.get("P119.3")?.status)
    && roadmapById.get("P119.2")?.status === "complete",
  `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`,
);
addCheck(
  "changed files stay in P119.2 allowed scope",
...
addCheck(
  ...
  enforceCurrentDiffScope ? changed.join(", ") : `P119.2 forbidden path check relaxed for ${status.currentPhase}`,
);
...
writeMarkdownReport(
  ...
  [
    ...
    { title: "Checks", body: buildCheckTable(checks) },
    {
      ...
      body: p1192.validationCommands.map((command) => `- ${command}`).join("\n"),
    },
    {
      title: "Known Limitations",
      body: "- P119.2 is schema metadata only. It does not create DB tables, write runtime records, capture approvals, persist approvals, record approve/reject decisions, unlock execution, run runtime work, call providers/models, dispatch agents, execute workers/tools, mutate projects, deploy, release, export, package, use network calls, or spend.",
    },
    { title: "Result", body: failed.length === 0 ? `PASS (${checks.length}/${checks.length})` : `FAIL (${failed.length} failed)` },
  ],
  { title: "P119.2 Founder Runtime Approval Decision Recording Boundary Schema Report", phase: "P119.2" },
);

printCheckReport("P119.2 Founder Runtime Approval Decision Recording Boundary Schema Check", checks, failed.length === 0 ? "PASS" : "FAIL");
