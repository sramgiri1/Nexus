...
addCheck("phase status starts or safely hands off P135.5", p1355CurrentState || p1356CurrentState || p1357FinalState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P135.6 remains planned or safely handed off", (statusById.get("P135.6")?.status === "planned" && roadmapById.get("P135.6")?.status === "planned" && !(statusById.get("P135.6")?.checksRun || []).length) || p1356CurrentState || p1357FinalState);
