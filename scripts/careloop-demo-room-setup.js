...
const careloopProjectDir = path.join(repoRoot, "projects", "careloop");
const careloopIOSProject = path.join(repoRoot, "projects", "careloop-ios", "CareLoop.xcodeproj");
...
function seedManifest() {
  const raw = run(
    "node",
    [path.join(careloopProjectDir, "scripts", "seed-demo-showcase.js")],
    { cwd: careloopProjectDir },
  );
...
async function refreshManifestTokens(manifest) {
  ...
  for (const email of emails) {
    const payload = await postJson(`${apiBaseUrl}/auth/login`, {
      email,
      password,
    });
...
function hasInstalledApp(device) {
  ...
  return appContainer.length > 0 && fs.existsSync(appContainer);
}
...
async function installApp(device, appPath) {
  ...
  if (!forceInstall && hasInstalledApp(device)) {
    return;
  }
