...
addCheck("README records P136.7", /P136\.7 secrets\/providers\/tool governance final validation/i.test(readme) && /P137 agent work order runtime is planned-only next/i.test(readme));
addCheck("platform roadmap records P136.7", /P136\.7 secrets\/providers\/tool governance final validation is complete/i.test(platformRoadmap) && /P137 Agent Work Order Runtime is planned-only next/i.test(platformRoadmap));
addCheck("enterprise roadmap records P136 closure", /P136\.1 through P136\.7\s+are\s+now complete/i.test(enterpriseRoadmap) && /P137 is the next executable phase/i.test(enterpriseRoadmap));
addCheck("phase status closes P136", p1367FinalState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
...
addCheck("P137 handoff remains safe", p137Status.status === "planned" && p137Status.commit === "" && Array.isArray(p137Status.checksRun) && p137Status.checksRun.length === 0 && (p137Status.knownLimitations || []).join(" ").toLowerCase().includes("planned-only"));
addCheck(
