...
const REPORT_PATH = "reports/p1456-enterprise-ga-readiness-docs-roadmap-report.md";
...
const REQUIRED_SCRIPT = "check:p1456-enterprise-ga-readiness-docs-roadmap";
const PRIOR_SCRIPT = "check:p1455-enterprise-ga-readiness-tests";
const NEXT_SCRIPT = "check:p1457-enterprise-ga-readiness-final-validation";
const EXPECTED_BASE_COMMIT = "6552ba33";
const PRIOR_REPORTS = [
...
const checkerSource = readText("scripts/check-p1456-enterprise-ga-readiness-docs-roadmap.js");
const p1455Checker = readText("scripts/check-p1455-enterprise-ga-readiness-tests.js");
...
const enforceCurrentDiffScope = status.currentPhase === "P145.6";
const allowedFiles = new Set(p1456.allowedFiles || []);
...
addCheck("package script registered", packageJson.scripts?.[REQUIRED_SCRIPT] === "node scripts/check-p1456-enterprise-ga-readiness-docs-roadmap.js");
addCheck("P145.7 final checker registered when final state", !p1457FinalState || packageJson.scripts?.[NEXT_SCRIPT] === "node scripts/check-p1457-enterprise-ga-readiness-final-validation.js");
addCheck("checker reuses shared report helpers", checkerSource.includes("../shared/reportWriter.js") && checkerSource.includes("../shared/checkResultFormatter.js"));
addCheck("P145.1-P145.5 reports pass", PRIOR_REPORTS.every(reportPassed), `${PRIOR_REPORTS.filter(reportPassed).length}/${PRIOR_REPORTS.length}`);
addCheck("P145.5 checker accepts P145.6", p1455Checker.includes("p1456CurrentState") && p1455Checker.includes(REQUIRED_SCRIPT) && p1455Checker.includes("P145.6 handoff remains valid"));
addCheck("enterprise checker accepts P145.6", enterpriseChecker.includes("p1456CurrentState") && enterpriseChecker.includes(REQUIRED_SCRIPT));
addCheck("OS checker recognizes P145.7 handoff", osStatusChecker.includes('"P145.6"') && osStatusChecker.includes('"P145.7"'));
addCheck("contract marks P145.6 complete", contract.phaseId === "P145" && p1456.status === "complete" && ((contract.status === "in_progress" && contract.currentSubphase === "P145.6" && contract.previousSubphase === "P145.5" && contract.nextSubphase === "P145.7" && p1457.status === "planned") || p1457FinalState));
addCheck("contract records expected base commit", p1456.expectedBaseCommit === EXPECTED_BASE_COMMIT);
addCheck("contract records validation commands", VALIDATION_COMMANDS.every((command) => p1456.validationCommands?.includes(command)));
addCheck("contract scope stays docs/status-only", /docs|roadmap|status/i.test(p1456.dataShape || "") && p1456.expectedExports?.length === 0 && p1456.forbiddenFiles?.includes("projects/**") && p1456.forbiddenFiles?.includes("db/**"));
...
addCheck("docs record P145.6", /## P145\.6 Docs \/ Roadmap \/ Status[\s\S]*Status:\s+complete/.test(plan) && /P145\.6 Docs \/ Roadmap \/ Status is complete/i.test(readme) && /P145\.6 docs\/status closure is complete/i.test(platformRoadmap) && /P145\.6 is now complete/i.test(enterpriseRoadmap));
addCheck("phase status starts or safely hands off P145.6", p1456CurrentState || p1457FinalState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
addCheck("completed P145.6 entries have required fields", [p145, statusById.get("P145.6"), p145Roadmap, roadmapById.get("P145.6")].every((entry) => Boolean(entry?.phaseId) && Boolean(entry.branch) && Boolean(entry.commit) && Boolean(entry.summary) && Array.isArray(entry.checksRun) && entry.checksRun.includes(`npm run ${REQUIRED_SCRIPT}`) && Array.isArray(entry.knownLimitations) && entry.commandCenterVisible === true));
addCheck("P145.7 handoff remains valid", (p1456CurrentState && statusById.get("P145.7")?.status === "planned" && roadmapById.get("P145.7")?.status === "planned" && !(statusById.get("P145.7")?.checksRun || []).length && !(roadmapById.get("P145.7")?.checksRun || []).length) || p1457FinalState);
addCheck("P145.6 Playwright coverage exists", routeTests.includes("P145.6 docs status keeps enterprise GA readiness roadmap current") && routeTests.includes("P145.7") && routeTests.includes("Final Validation") && routeTests.includes("Enterprise GA Readiness"));
...
addCheck("changed files stay in P145.6 allowed scope", !enforceCurrentDiffScope || changed.every((file) => allowedFiles.has(file)), enforceCurrentDiffScope ? changed.join(", ") : `scope check relaxed for ${status.currentPhase}`);
addCheck("forbidden paths unchanged", !enforceCurrentDiffScope || changed.every((file) => !forbiddenPrefixes.some((prefix) => file === prefix.replace(/\/$/, "") || file.startsWith(prefix)) || file === "dashboard/tests/routes.spec.js"), enforceCurrentDiffScope ? changed.join(", ") : `scope check relaxed for ${status.currentPhase}`);
  ...
  [
    ...
    },
    {
      ...
      body: [
        ...
        `- Next subphase: ${status.nextPhase || "none"}`,
