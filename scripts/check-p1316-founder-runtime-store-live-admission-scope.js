...
const REPORT_PATH = "reports/p1316-founder-runtime-store-live-admission-scope-report.md";
...
function hasUnsafePositiveClaim(text, pattern) {
  ...
  return lines.some((line, index) => {
    ...
    return pattern.test(line)
      && !/\b(no|not|never|without|blocked|unavailable|disabled|forbidden|do not|does not|remain|remains|safe dry-run|dry-run-only|planned-only|display-only|read-only|future|local-only|model-only|request model|request shape|readiness resolver|resolver only|readiness only|dry-run model|dry run|ux only|display-safe|validation\/docs|validation-only|docs-only|cannot)\b/i.test(context);
  });
}
...
const p1315Checker = readText("scripts/check-p1315-founder-runtime-store-live-admission-scope.js");
const checkerSource = readText("scripts/check-p1316-founder-runtime-store-live-admission-scope.js");
...
const enforceCurrentDiffScope = status.currentPhase === "P131.6";
const allowedFiles = new Set(p1316.allowedFiles || []);
...
const completedSubphases = ["P131.1", "P131.2", "P131.3", "P131.4", "P131.5", "P131.6"];
...
addCheck(
  "contract marks P131.6 complete",
...
addCheck("P131.6 records expected base commit", p1316.expectedBaseCommit === "555153df");
addCheck("P131.7 remains planned or complete", ["planned", "complete"].includes(p1317.status));
addCheck("P131.6 allowed files include checker and reports", p1316.allowedFiles?.includes("scripts/check-p1316-founder-runtime-store-live-admission-scope.js") && p1316.allowedFiles?.includes(REPORT_PATH) && p1316.allowedFiles?.includes("scripts/check-p1315-founder-runtime-store-live-admission-scope.js") && p1316.allowedFiles?.includes("reports/p1315-founder-runtime-store-live-admission-scope-report.md"));
addCheck("P131.6 forbids dashboard/project/db/runtime paths", ["projects/**", "careloop/**", "generated-projects/**", "dashboard/src/**", "dashboard/tests/**", "db/**", "local-state/runtime/**", "providers/**", "tools/**", "worker-runtime/**"].every((path) => p1316.forbiddenFiles?.includes(path)));
addCheck("P131.6 records validation commands", validationCommands.every((command) => p1316.validationCommands?.includes(command)));
addCheck("P131.1-P131.6 contract entries complete", completedSubphases.every((phaseId) => subphaseById.get(phaseId)?.status === "complete"));
addCheck("P131.1-P131.5 reports pass", previousReports.every(reportPassed));
addCheck("P131.5 checker accepts P131.6 handoff", p1315Checker.includes("p1316HandoffState") && p1315Checker.includes('status.currentPhase === "P131.6"') && p1315Checker.includes('status.nextPhase === "P131.7"'));
...
addCheck("P131 plan records P131.6", /## P131\.6 Validation \/ Docs[\s\S]*Status:\s+complete/.test(plan));
addCheck("README records P131.6", /P131\.6 validation\/docs consolidation/i.test(readme));
addCheck("platform roadmap records P131.6", /P131\.6 is complete/i.test(platformRoadmap) && (/P131\.7\s+is\s+next/i.test(platformRoadmap) || /P131\.7\s+is\s+complete/i.test(platformRoadmap)));
...
addCheck("phase status advanced", p1316HandoffState || p1317FinalState, `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`);
addCheck(
...
addCheck("completed P131.6 entries have required fields", [statusById.get("P131"), statusById.get("P131.6"), roadmapById.get("P131.6")].every((entry) => Boolean(entry?.phaseId) && Boolean(entry.branch) && Boolean(entry.commit) && Boolean(entry.summary) && Array.isArray(entry.checksRun) && Array.isArray(entry.knownLimitations)));
addCheck(
  "changed files stay in P131.6 allowed scope",
...
addCheck(
  ...
  enforceCurrentDiffScope ? changed.join(", ") : `P131.6 forbidden path check relaxed for ${status.currentPhase}`,
);
addCheck("P131.6 contract avoids forbidden file scope", !(p1316.allowedFiles || []).some((file) => forbiddenPrefixes.some((prefix) => file.startsWith(prefix))));
...
writeMarkdownReport(
  ...
  [
    ...
    { title: "Validation Commands", body: p1316.validationCommands.map((command) => `- ${command}`).join("\n") },
    {
      title: "Known Limitations",
      body: "- P131.6 is validation/docs closure only. It does not capture approvals, persist decisions, submit requests, persist requests, create DB schemas, run migrations, read or write DB/runtime records, persist acceptance capture, run CRUD actions, capture acceptance, accept handoff, hand off authority, grant authority, activate authority, apply approvals, record approve/reject decisions, unlock execution, call providers/models, dispatch agents, mutate projects, deploy, release, export, package, use network calls, or spend.",
    },
    { title: "Result", body: failed.length === 0 ? `PASS (${checks.length}/${checks.length})` : `FAIL (${failed.length} failed)` },
  ],
  { title: "P131.6 Store Live Admission Scope Validation / Docs Report", phase: "P131.6" },
);

printCheckReport("P131.6 Store Live Admission Scope Validation / Docs Check", checks, failed.length === 0 ? "PASS" : "FAIL");
