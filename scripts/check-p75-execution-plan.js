...
const CONTRACT_PATH = "contracts/os-roadmap/p75-execution-contracts.json";
const PLAN_PATH = "docs/architecture/P75_BACKUP_RESTORE_DR_PLAN.md";
const STATUS_PATH = "os-roadmap/phase-status.json";
const REPORT_PATH = "reports/p75-execution-plan-report.md";

export const P75_BACKUP_DR_SUBPHASES = ["P75.1", "P75.2", "P75.3", "P75.4", "P75.5", "P75.6", "P75.7"];
...
export function checkP75ExecutionPlan() {
  const checks = {
    contractFile: true,
    taskContracts: true,
    safetyRules: true,
  ...
  if (tasks.length !== P75_BACKUP_DR_SUBPHASES.length) fail("taskContracts", `Expected ${P75_BACKUP_DR_SUBPHASES.length} task contracts, found ${tasks.length}`);
  ...
  for (const task of tasks) {
    ...
    if (!hasAllText(inputs.safetyRules || [], ["backup", "restore", "delete", "DB writes", "project mutation", "provider", "tool execution", "network", "provider spend"])) {
      fail("safetyRules", `${task.id} safety rules must block backup/restore/delete, DB writes, project mutation, provider/tool execution, network calls, and spend`);
    }
    ...
    if (!(inputs.validationCommands || []).includes("npm run check:p75-execution-plan")) fail("validationCommands", `${task.id} must include npm run check:p75-execution-plan`);
  }
  for (const phaseId of P75_BACKUP_DR_SUBPHASES) {
  ...
  const p75 = phaseStatus.get("P75");
  const expectedNext = P75_BACKUP_DR_SUBPHASES.find((phaseId) => phaseStatus.get(phaseId)?.status !== "complete") || "P76";
  const finalHandoff = p75?.status === "complete" && status.currentPhase === "P76" && status.previousPhase === "P75" && status.nextPhase === "P76";
  if (!["in_progress", "complete"].includes(p75?.status)) fail("roadmapStatus", "P75 must be in_progress or complete");
  if (p75?.nextPhase !== expectedNext) fail("roadmapStatus", `P75 nextPhase must be ${expectedNext}`);
  if (!finalHandoff && status.currentPhase !== "P75") fail("roadmapStatus", "currentPhase must be P75 before final handoff");
  if (!finalHandoff && status.previousPhase !== "P74") fail("roadmapStatus", "previousPhase must be P74 before final handoff");
  ...
  if (!/backup[\s\S]+restore[\s\S]+remain disabled/i.test(plan) || !/restore execution[\s\S]+disabled/i.test(plan)) {
    fail("docs", `${PLAN_PATH} must state backup and restore execution remain disabled`);
  }
  ...
  writeMarkdownReport(
    REPORT_PATH,
    [
      { title: "Contract", body: [`- Path: ${CONTRACT_PATH}`, `- Subphases: ${P75_BACKUP_DR_SUBPHASES.join(", ")}`].join("\n") },
    ...
    { title: "P75 Execution Plan Report", phase: "P75.1" },
  );
  return { rows, failures, result: failures.length === 0 ? "PASS" : "FAIL" };
}

const result = checkP75ExecutionPlan();
printCheckReport("P75 Execution Plan Check", result.rows, result.result);
