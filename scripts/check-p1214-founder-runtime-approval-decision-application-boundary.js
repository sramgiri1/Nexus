...
const REPORT_PATH = "reports/p1214-founder-runtime-approval-decision-application-boundary-report.md";
...
const p1213Checker = readText("scripts/check-p1213-founder-runtime-approval-decision-application-boundary.js");
const previewSource = readText("shared/founderApprovalDecisionApplicationPreview.js");
const preview = buildFounderApprovalDecisionApplicationPreview({
  ...
  nextAction: "Render P121.5 scoped application boundary UX without apply, approve, reject, save, or execution controls.",
});
const validation = validateFounderApprovalDecisionApplicationPreview(preview);
const data = preview.data || {};
const summary = data.approvalDecisionApplicationSummary || {};
const invalidValidation = validateFounderApprovalDecisionApplicationPreview({
  ...preview,
  data: {
    ...data,
    dryRunOnly: false,
...
const enforceCurrentDiffScope = status.currentPhase === "P121.4";
const allowedFiles = new Set(p1214.allowedFiles || []);
...
const blockedFlagNames = [
  ...
  "approvalDecisionApplicationPreviewAllowed",
  "approvalDecisionApplicationPreviewWritten",
...
const p1214HandoffAccepted = (status.currentPhase === "P121.4"
...
addCheck("package script registered", Boolean(packageJson.scripts?.["check:p1214-founder-runtime-approval-decision-application-boundary"]));
addCheck("preview exports exist", previewSource.includes("buildFounderApprovalDecisionApplicationPreview") && previewSource.includes("validateFounderApprovalDecisionApplicationPreview"));
addCheck("phase and version exports", FOUNDER_APPROVAL_DECISION_APPLICATION_PREVIEW_PHASE === "P121.4" && FOUNDER_APPROVAL_DECISION_APPLICATION_PREVIEW_VERSION === "1.0");
addCheck("preview states are allowlisted", Object.values(FOUNDER_APPROVAL_DECISION_APPLICATION_PREVIEW_STATES).includes("application preview ready; application blocked") && !Object.values(FOUNDER_APPROVAL_DECISION_APPLICATION_PREVIEW_STATES).includes("approval_decision_application_live"));
...
addCheck("preview envelope shape", preview.ok === true && preview.status === "PASS" && data.schemaVersion === "1.0" && data.previewMode === "local-only-application-dry-run" && data.dryRunOnly === true);
addCheck("preview stays Command Center hidden", data.commandCenterVisible === false);
addCheck("preview reuses P121.3 intent model and P121.2 metadata", data.sourceIntentPhase === "P121.3" && data.sourceMetadataPhase === "P121.2" && previewSource.includes("buildFounderApprovalDecisionApplicationIntentModel") && previewSource.includes("buildFounderApprovalDecisionApplicationEligibilityMetadata"));
...
addCheck("row authority flags false", data.previewRows?.every((row) => flagsFalse(row) && row.wouldApplyDecision === false && row.wouldCaptureApproval === false && row.wouldPersistApproval === false && row.wouldRecordDecision === false && row.wouldAcceptDecision === false && row.wouldRejectDecision === false && row.wouldWriteDb === false && row.wouldWriteRuntime === false && row.wouldUnlockExecution === false && row.wouldDispatchAgent === false && row.wouldMutateProject === false && row.wouldSpend === false));
...
addCheck("contract marks P121.4 complete", p1214.status === "complete" && ["planned", "complete"].includes(p1215.status) && ["planned", "complete"].includes(p1216.status));
addCheck("contract records expected exports", [
  "FOUNDER_APPROVAL_DECISION_APPLICATION_PREVIEW_PHASE",
  "buildFounderApprovalDecisionApplicationPreview",
  "validateFounderApprovalDecisionApplicationPreview",
].every((name) => p1214.expectedExports?.includes(name)));
addCheck("P121.3 checker accepts P121.4 handoff", p1213Checker.includes("P121.4") && p1213Checker.includes("P121.5") && p1213Checker.includes("p1214StartedState"));
addCheck("docs record P121.4", /P121\.4 Application Safe Dry Run[\s\S]*Status:\s+complete/.test(plan));
addCheck("README records P121.4", /P121\.4 approval decision application safe dry run/i.test(readme) && /P121\.5\s+is\s+next/.test(readme));
addCheck("platform roadmap records P121.4", /P121\.4 is complete/.test(platformRoadmap) && /P121\.5\s+is\s+next/.test(platformRoadmap));
addCheck(
  "phase status advanced",
  p1214HandoffAccepted
    && statusById.get("P121")?.status === "in_progress"
    && statusById.get("P121.3")?.status === "complete"
    && statusById.get("P121.4")?.status === "complete"
    && ["planned", "complete"].includes(statusById.get("P121.5")?.status)
    && roadmapById.get("P121.4")?.status === "complete",
  `${status.currentPhase}/${status.previousPhase}/${status.nextPhase}`,
);
addCheck(
  "changed files stay in P121.4 allowed scope",
...
addCheck(
  ...
  enforceCurrentDiffScope ? changed.join(", ") : `P121.4 forbidden path check relaxed for ${status.currentPhase}`,
);
addCheck("public docs avoid raw application table names", !/(founder_runtime_approval_application|approval_decision_application_records|approval_decision_application_events|approval_decision_application_requests|founderApprovalDecisionApplication)/i.test(publicDocsBundle));
...
writeMarkdownReport(
  ...
  [
    ...
    { title: "Validation Commands", body: p1214.validationCommands.map((command) => `- ${command}`).join("\n") },
    {
      title: "Known Limitations",
      body: "- P121.4 is a local dry-run preview only. It does not apply approvals, accept approvals, persist approvals, record approve/reject decisions, write DB/runtime records, unlock execution, run runtime work, call providers/models, dispatch agents, execute workers/tools, mutate projects, deploy, release, export, package, use network calls, or spend.",
    },
    { title: "Result", body: failed.length === 0 ? `PASS (${checks.length}/${checks.length})` : `FAIL (${failed.length} failed)` },
  ],
  { title: "P121.4 Founder Runtime Approval Decision Application Boundary Safe Dry Run Report", phase: "P121.4" },
);

printCheckReport("P121.4 Founder Runtime Approval Decision Application Boundary Safe Dry Run Check", checks, failed.length === 0 ? "PASS" : "FAIL");
