...
| Task detail / comments / snooze | `test_taskDetailCanSnoozeReminder`; `test_taskDetailCanChangeStatusToDone`; `test_taskCommentsCanBeAddedAndDeleted`; Model tests; backend hidden comment mutation tests | Snooze destructive/edge-state copy | None | Comment editing is not in current scope |
