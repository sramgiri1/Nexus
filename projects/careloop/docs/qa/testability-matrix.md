...
| Pending invites / join circle | `test_careReceiverCanAcceptPendingInviteFromDirectory`; backend invite accept/decline/expiry tests | Pending invite cards, expired/declined/revoked disabled states | Email link open into app | Universal-link invite acceptance automation |
...
| People and access | `test_organizerCanOpenPeopleAndAccess`; `test_organizerCanResendAndRevokePendingCaregiverInvite`; backend invite resend/revoke and access grant/revoke tests | Invite caregiver sheet, access toggles, role changes | Real invite email delivery | UI automation for grant receiver access after caregiver acceptance |
| Care receiver management | `test_organizerCanOpenCareReceiverManagement`; backend invite resend/revoke, add/reorder/delete/proxy tests | Add/edit/reorder/proxy activate sheets, receiver invite resend/revoke | Consent/legal review on real device | UI automation for invite receiver and proxy activation |
