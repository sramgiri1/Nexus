...
| Premium paywall / purchase | `test_organizerCanOpenReceiverPremiumPaywall`; `test_organizerCanManagePremiumReceiverPlan`; `CareLoopTests/SubscriptionManagerProductIdTests` product metadata and StoreKit fixture parity coverage; backend entitlement source/transaction/product validation; `xcodebuild build-for-testing` for restore sync refactors | Paywall layout, entitlement states | Sandbox purchase, restore, refund/revoke | Server-side App Store receipt verification automation |
