...
| Invite member / invite care receiver | `backend:circles`, `ios:personas`; focused iOS UI coverage for pending caregiver invite resend/revoke and direct care receiver invite acceptance |
...
- Focused pending-invite validation can be run with Xcode UI tests `test_organizerCanResendAndRevokePendingCaregiverInvite` and `test_careReceiverCanAcceptPendingInviteFromDirectory`.
