...
| Invite member / invite care receiver | `backend:circles`, `ios:personas`; focused iOS UI coverage for pending caregiver invite resend/revoke, direct care receiver invite acceptance, and caregiver no-access default join |
...
- Focused pending-invite validation can be run with Xcode UI tests `test_organizerCanResendAndRevokePendingCaregiverInvite`, `test_careReceiverCanAcceptPendingInviteFromDirectory`, and `test_caregiverAcceptsInviteWithNoReceiverAccessByDefault`.
