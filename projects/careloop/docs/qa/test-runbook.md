...
| Care receiver lifecycle management | Backend receiver lifecycle tests; focused iOS UI `test_organizerCanAddEditAndRemoveCareReceiverLocally` and `test_addSecondReceiverShowsPremiumGateBeforeForm` |
...
- Focused receiver lifecycle validation can be run with backend receiver tests in `test/sprint2.test.js` and Xcode UI tests `test_addSecondReceiverShowsPremiumGateBeforeForm` and `test_organizerCanAddEditAndRemoveCareReceiverLocally`.
