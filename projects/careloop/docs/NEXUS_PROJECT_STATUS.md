...
- Project: CareLoop
- Active phase: CARELOOP-K-STANDALONE-SEPARATION
- Mission: CareLoop field gap closure, reliability, and release readiness
- Status: Premium subphases P1-P8, post-premium Phase A, Phase B invite flow, Phase C receiver management polish, Phase D1-D5 StoreKit/billing/free-policy hardening, Phase E1-E5 reports/insights, Phase F1-F4 notification/deep-link/snooze/escalation coverage, Phase G demo readiness/persona recording, Phase H1-H3 local release hygiene, Phase I0 scenario reliability/API-backed test harness, Phase J1 scale hot-path hardening, Phase K1 standalone command-boundary work, and Phase K2A standalone export tooling implemented and validation complete on the active branch
- Next action: K2B physical standalone repo extraction can start once the target repository/path is chosen. J2 scheduler/queue readiness and I1 shared care calendar can also start locally if product scope takes priority. H4/H5 production API, App Store Connect, Sign in with Apple, APNs, StoreKit sandbox, TestFlight, and physical-device validation wait on external Apple/backend setup.
...
- Phase K1 is complete: CareLoop-local build/test/demo commands no longer depend on a Nexus-root demo launcher, iOS path resolution supports standalone and current sibling layouts, and standalone build/test documentation defines how to extract without app-source or secret leakage.
- Phase K2A is complete: standalone export tooling can create a clean CareLoop app-only layout with backend, iOS, docs, tests, Prisma, and scripts while excluding local secrets, runtime artifacts, Nexus OS files, dashboard, reports, roadmap, and generated simulator output.
...
- `npm run check:demo-showcase`, `npm test`, and iOS path resolver smoke validation passed after Phase K1 standalone command-boundary work.
- `npm run check:standalone-export` and a guarded temporary standalone export validation passed after Phase K2A export tooling.
